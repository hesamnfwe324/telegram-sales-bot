#!/bin/bash
  ...
  # Fix DATABASE_URL format for asyncpg
  ...
  # Unset PYTHONPATH so local alembic/ folder doesn't shadow the installed package
  env -u PYTHONPATH alembic upgrade head || echo "[warn] Migration warning — may already be up to date"
