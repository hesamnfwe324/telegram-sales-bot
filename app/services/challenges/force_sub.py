"""
    ...
    4. When the user clicks "✅ بررسی عضویت" call:
    ...
    getChatMember notes
    -------------------
    • Public channels (@username is known):
      → Works WITHOUT bot being an admin. Use @username.
    • Private channels (no username, only numeric ID):
    ...
    # {user_id: (monotonic_ts, allowed, statuses)}
    ...
    def invalidate_cache(user_id: int) -> None:
      ...
      Call this before re-checking membership so the bot contacts Telegram
      fresh instead of returning the old cached answer.
      """
    ...
    async def _check_one(
      ...
      # Build the list of chat_id values to try.
      # @username works for public channels without the bot being an admin.
      # Numeric ID works when the bot IS an admin (private channels too).
    ...
    async def check_membership(
      ...
      *,
      ...
      force:    If True, bypass the cache and re-check with Telegram.
      ...
      # Return cached result unless force=True or cache is stale
      ...
      if not channels:
          # No channels configured yet — keep gate closed with a generic message
    ...
    def build_gate_message(statuses: list[dict[str, str | None]]) -> tuple[str, InlineKeyboardMarkup]:
      """Build the gate message text and keyboard from channel statuses."""
      lines = [
          "<b>⛔ عضویت اجباری</b>",
          "",
      ...
      # Folder link as fallback when individual links are missing
    ...
    async def send_gate(message: Message, statuses: list[dict[str, str | None]]) -> None:
