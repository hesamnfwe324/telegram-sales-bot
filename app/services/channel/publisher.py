...
async def set_challenge_buttons(
    channel_id: str | int,
    message_id: int,
    challenge_id: str,
    options: list[str],
) -> None:
    """Attach answer buttons to a published challenge message.

    The admin bot owns these buttons, while the userbot remains responsible for
    posting into the channel. Only the short challenge UUID and option index are
    placed in callback data.
    """
    from app.core.config import settings

    bot_token = settings.ADMIN_BOT_TOKEN
    if not bot_token or len(options) != 4:
        return
    try:
        async with httpx.AsyncClient(timeout=8) as http:
                ...
                json={
                    ...
                    "reply_markup": {
                        "inline_keyboard": [
                            [
                                {
                                    "text": f"{index + 1}. {option[:38]}",
                                    "callback_data": f"rdp_ch:{challenge_id}:{index}",
                                }
                            ]
            ...
            if not data.get("ok"):
                logger.warning(
                    "challenge_buttons_failed",
                    channel_id=str(channel_id),
                    reason=data.get("description"),
                )
    except Exception as exc:
        logger.warning(
            "challenge_buttons_exception",
            channel_id=str(channel_id),
            error=repr(exc),
        )
