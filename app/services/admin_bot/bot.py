import asyncio
  ...
  async def _ensure_webhook(bot: Bot, webhook_url: str, delay: float = 20.0) -> None:
      """Re-verify and re-set the webhook after a short delay.

      Guards against the deploy race condition where the OLD instance's
      shutdown_admin_bot() runs AFTER the new instance has already called
      set_webhook, wiping the webhook and leaving the bot unreachable.
      """
  ...
  async def setup_admin_bot(userbot_manager=None) -> None:
      ...
      if webhook_url:
          ...
          # Clear any old webhook first, then register the new one.
          try:
          ...
          try:
              ...
              # Re-verify after 20s to recover from old-deploy shutdown race
  ...
  async def shutdown_admin_bot() -> None:
      """Graceful shutdown — intentionally does NOT delete the webhook.

      Removing the webhook here would break rolling deploys: the new instance
      calls set_webhook before this instance shuts down, and a delete_webhook
      here would wipe the new instance's registration.
      The new instance always calls delete_webhook + set_webhook at startup,
