...
_rdp_challenge_task: asyncio.Task | None = None
...
async def _background_init() -> None:
    global _userbot_task, _auto_poster_task, _rdp_challenge_task
    ...
    # The legacy auto-poster and RDP scanner are intentionally not started.
    # The challenge campaign below uses documentation-only examples and does
    # not scan networks, test credentials, or publish third-party access data.

    try:
        from app.services.challenges.service import run_challenge_campaign_loop
        _rdp_challenge_task = asyncio.create_task(run_challenge_campaign_loop())
    except Exception as e:
        _startup_errors.append(f"rdp_challenge_loop: {e}")
...
async def lifespan(app: FastAPI):
    ...
    for task in (_rdp_pool_task, _auto_poster_task, _userbot_task, _rdp_challenge_task):
        if task:
            task.cancel()
            try:
                await task
